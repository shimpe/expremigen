...
class PhraseFactory:
    ...
    def playeddur(cls, notes, type="l2s"):
        ...
        """
        if type == "l2s":
            anim = Pseq([Pconst(LEGATO, len(notes) - 1), Pconst(STACCATO, 1)], 1)
        elif type == "s2l":
            anim = Pseq([Pconst(STACCATO, len(notes) - 1), Pconst(LEGATO, 1)], 1)
        elif type == "s2s":
            anim = Pconst(STACCATO, len(notes))
        elif type == "l2l":
            anim = Pconst(LEGATO, len(notes))
        else:
            anim = None
            assert False
        return anim
